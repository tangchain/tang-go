...
This is a small tool, written in Go, for working with `tang-core` history archives directly.
It is a standalone tool that does not require `tang-core`, or any other programs.

It is much smaller and simpler than `tang-core`, and is intended only for archive-maintenance tasks.
...
```
$ go install github.com/tang/go/tools/tang-archivist
```

## Usage

```
inspect tang history archive

Usage:
  tang-archivist [flags]
  tang-archivist [command]
...
Flags:
  ...
  -h, --help              help for tang-archivist
...
Use "tang-archivist [command] --help" for more information about a command.
```

## Specifying history archives

Unlike `tang-core`, `tang-archivist` does not run subprocesses to access history archives;
...
`tang-archivist` supports reading from and writing to any S3-compatible storage.
...
```
$ tang-archivist status --s3endpoint ams3.digitaloceanspaces.com s3://bucketname/prefix
```
...
```
$ tang-archivist status http://s3-eu-west-1.amazonaws.com/history.tang.org/prd/core-testnet/core_testnet_001

       Archive: http://s3-eu-west-1.amazonaws.com/history.tang.org/prd/core-testnet/core_testnet_001
...
```
$ tang-archivist mirror http://s3-eu-west-1.amazonaws.com/history.tang.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 18:27:09 mirroring http://s3-eu-west-1.amazonaws.com/history.tang.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ tang-archivist --last 1024 mirror http://s3-eu-west-1.amazonaws.com/history.tang.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 19:14:01 mirroring http://s3-eu-west-1.amazonaws.com/history.tang.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ tang-archivist scan file://local-archive
...
```
$ tang-archivist --last 4096 scan file://local-archive
...
$ tang-archivist --low 1000000 --high 1006000 scan file://local-archive
...
```
$ tang-archivist --verify --last 4096 scan file://local-archive
...
$ tang-archivist repair file://local-archive file://broken-archive
...
```
 tang-archivist dumpxdr local-archive/transactions//00/20/de/transactions-0020de7f.xdr.gz
