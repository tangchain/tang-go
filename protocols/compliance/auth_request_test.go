...
func TestValidateError(t *testing.T) {
	...
	authData := &AuthData{
		Sender:         "bartektang.org",
		NeedInfo:       false,
		Tx:             "&^%",
		AttachmentJSON: "abc",
	}

	assert.EqualError(t, authData.Validate(), "sender: bartektang.org does not validate as tang_address;tx: &^% does not validate as base64;attachment: abc does not validate as json")
}
