...
// to sender server in order to obtain tang.toml file and signing key.
func (r *AuthRequest) VerifySignature(sender string) error {
	...
	senderTangToml, err := tangtoml.GetTangTomlByAddress(sender)
	if err != nil {
		return errors.Wrap(err, "Cannot get tang.toml of sender domain")
	}

	if senderTangToml.SigningKey == "" {
		return errors.New("No SIGNING_KEY in tang.toml of sender")
	}

	kp, err := keypair.Parse(senderTangToml.SigningKey)
	if err != nil {
