...
* ["Trades"](https://www.tang.org/developers/horizon/reference/endpoints/trades.html) endpoint can now be streamed.
* In ["Trade Aggregations"](https://www.tang.org/developers/horizon/reference/endpoints/trade_aggregations.html) endpoint, `offset` parameter has been added.
* Path finding bugs have been fixed and the algorithm has been improved. Check [#719](https://github.com/tang/go/pull/719) for more information.
...
* In ["Offers for Account"](https://www.tang.org/developers/horizon/reference/endpoints/offers-for-account.html), `last_modified_time` field  endpoint can be `null` when ledger data is not available (has not been ingested yet).
* ["Trades for Offer"](https://www.tang.org/developers/horizon/reference/endpoints/trades-for-offer.html) endpoint will query for trades that match the given offer on either side of trades, rather than just the "sell" offer. Offer IDs are now [synthetic](https://www.tang.org/developers/horizon/reference/resources/trade.html#synthetic-offer-ids). You have to reingest history to update offer IDs.
...
* Assets stats can cause high CPU usage on tang-core DB. If this slows down the database it's now possible to turn off this feature by setting `DISABLE_ASSET_STATS` feature flag. This can be set as environment variable (`DISABLE_ASSET_STATS=true`) or CLI parameter (`--disable-asset-stats=true`).
...
* Protocol V10 features:
  * New `bump_sequence` operation (as in [CAP-0001](https://github.com/tang/tang-protocol/blob/master/core/cap-0001.md)).
    * New [`bump_sequence`](https://www.tang.org/developers/horizon/reference/resources/operation.html#bump-sequence) operation.
    * New `sequence_bumped` effect.
    * Please check [CAP-0001](https://github.com/tang/tang-protocol/blob/master/core/cap-0001.md) for new error codes for transaction submission.
  * Offer liabilities (as in [CAP-0003](https://github.com/tang/tang-protocol/blob/master/core/cap-0003.md)):
    * `/accounts/{id}` resources contain new fields: `buying_liabilities` and `selling_liabilities` for each entry in `balances`.
    * Please check [CAP-0003](https://github.com/tang/tang-protocol/blob/master/core/cap-0003.md) for new error codes for transaction submission.
...
- `amount` field in `/assets` is now a String (to support Tang amounts larger than `int64`).
...
- Fixed an issue caused by un-migrated trade rows. (https://github.com/tang/go/issues/357)
...
- Changes to an account's inflation destination no longer produce erroneous "signer_updated" effects. (https://github.com/tang/horizon/issues/390)
...
- Upstream bug fixes to xdr decoding from `github.com/tang/go`.
...
### Fixed
- Ingestion was fixed to protect against text memos that contain null bytes.  While memos with null bytes are legal in tang-core, PostgreSQL does not support such values in string columns.  Horizon now strips those null bytes to fix the issue. 
...
### Added
- Horizon now exposes the tang network protocol in several places:  It shows the currently reported protocol version (as returned by the tang-core `info` command) on the root endpoint, and it reports the protocol version of each ledger resource.
- Trade resources now include a `created_at` timestamp.

### Fixed

- BREAKING CHANGE: The reingestion process has been updated.  Prior versions of horizon would enter a failed state when a gap between the imported history and the tang-core database formed or when a previously imported ledger was no longer found in the tang-core database.  This usually occurs when running tang-core with the `CATCHUP_RECENT` config option.  With these changed, horizon will automatically trim "abandonded" ledgers: ledgers that are older than the core elder ledger.
...
This release contains the initial implementation of the "Abridged History System".  It allows a horizon system to be operated without complete knowledge of the ledger's history.  With this release, horizon will start ingesting data from the earliest point known to the connected tang-core instance, rather than ledger 1 as it behaved previously.  See the admin guide section titled "Ingesting tang-core data" for more details.

### Added

- *Elder* ledgers have been introduced:  An elder ledger is the oldest ledger known to a db.  For example, the `core_elder_ledger` attribute on the root endpoint refers to the oldest known ledger stored in the connected tang-core database.
...
- Horizon now reports the last ledger ingested to tang-core using the `setcursor` command.
...
- BREAKING: Horizon can now import data from tang-core without the aid of the horizon-importer project.  This process is now known as "ingestion", and is enabled by either setting the `INGEST` environment variable to "true" or specifying "--ingest" on the launch arguments for the horizon process.  Only one process should be running in this mode for any given horizon database.
...
[v0.11.0]: https://github.com/tang/horizon/compare/v0.10.1...v0.11.0
[v0.10.1]: https://github.com/tang/horizon/compare/v0.10.0...v0.10.1
[v0.10.0]: https://github.com/tang/horizon/compare/v0.9.1...v0.10.0
[v0.9.1]: https://github.com/tang/horizon/compare/v0.9.0...v0.9.1
[v0.9.0]: https://github.com/tang/horizon/compare/v0.8.0...v0.9.0
[v0.8.0]: https://github.com/tang/horizon/compare/v0.7.1...v0.8.0
[v0.7.1]: https://github.com/tang/horizon/compare/v0.7.0...v0.7.1
[v0.7.0]: https://github.com/tang/horizon/compare/v0.6.2...v0.7.0
[v0.6.2]: https://github.com/tang/horizon/compare/v0.6.1...v0.6.2
[v0.6.1]: https://github.com/tang/horizon/compare/v0.6.0...v0.6.1
[v0.6.0]: https://github.com/tang/horizon/compare/v0.5.1...v0.6.0
[v0.5.1]: https://github.com/tang/horizon/compare/v0.5.0...v0.5.1
[v0.5.0]: https://github.com/tang/horizon/compare/v0.4.0...v0.5.0
[v0.4.0]: https://github.com/tang/horizon/compare/v0.3.0...v0.4.0
[v0.3.0]: https://github.com/tang/horizon/compare/v0.2.0...v0.3.0
[v0.2.0]: https://github.com/tang/horizon/compare/v0.1.1...v0.2.0
[v0.1.1]: https://github.com/tang/horizon/compare/v0.1.0...v0.1.1
