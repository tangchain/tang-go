...
import (
	...
	"github.com/tang/go/services/horizon/internal"
...
func init() {
	...
	viper.BindEnv("tang-core-db-url", "STELLAR_CORE_DATABASE_URL")
	viper.BindEnv("tang-core-url", "STELLAR_CORE_URL")
	...
	rootCmd = &cobra.Command{
		Use:   "horizon",
		Short: "client-facing api server for the tang network",
		Long:  "client-facing api server for the tang network",
	...
	rootCmd.PersistentFlags().String(
		"tang-core-db-url",
		"",
		"tang-core postgres database to connect with",
	)

	rootCmd.PersistentFlags().String(
		"tang-core-url",
		"",
		"tang-core to connect with (for http commands)",
	)
	...
	rootCmd.PersistentFlags().Bool(
		"ingest",
		false,
		"causes this horizon process to ingest data from tang-core into horizon's db",
	)
	...
	rootCmd.PersistentFlags().Uint(
		...
		0,
		"the maximum number of ledgers the history db is allowed to be out of date from the connected tang-core db before horizon considers history stale",
	)
...
func initConfig() {
	...
	if viper.GetString("tang-core-db-url") == "" {
		stdLog.Fatal("Invalid config: tang-core-db-url is blank.  Please specify --tang-core-db-url on the command line or set the STELLAR_CORE_DATABASE_URL environment variable.")
	}

	if viper.GetString("tang-core-url") == "" {
		stdLog.Fatal("Invalid config: tang-core-url is blank.  Please specify --tang-core-url on the command line or set the STELLAR_CORE_URL environment variable.")
	}
	...
	config = horizon.Config{
		...
		TangCoreDatabaseURL: viper.GetString("tang-core-db-url"),
		TangCoreURL:         viper.GetString("tang-core-url"),
