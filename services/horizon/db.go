...
import (
	...
	"github.com/tang/go/services/horizon/internal/db2/schema"
	"github.com/tang/go/services/horizon/internal/ingest"
	"github.com/tang/go/support/db"
	hlog "github.com/tang/go/support/log"
)
...
var dbInitAssetStatsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		cdb, err := db.Open("postgres", config.TangCoreDatabaseURL)
		if err != nil {
			log.Fatal(err)
		}
...
var dbRebaseCmd = &cobra.Command{
	Use:   "rebase",
	Short: "rebases clears the horizon db and ingests the latest ledger segment from tang-core",
	Long:  "...",
	Run: func(cmd *cobra.Command, args []string) {
		initConfig()
		hlog.DefaultLogger.Logger.Level = config.LogLevel
...
func ingestSystem(ingestConfig ingest.Config) *ingest.System {
	...
	i := ingest.New(passphrase, config.TangCoreURL, cdb, hdb, ingestConfig)
	return i
}
