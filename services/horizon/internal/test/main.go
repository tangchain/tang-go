...
import (
	...
	"github.com/tang/go/services/horizon/internal/logmetrics"
	tdb "github.com/tang/go/services/horizon/internal/test/db"
...
// LoadScenarioWithoutHorizon populates the test Tang core database a with
...
func Start(t *testing.T) *T {
	...
	result.CoreDB = TangCoreDatabase(t)
...
// TangCoreDatabase returns a connection to the tang core test database
//
// DEPRECATED:  use `TangCore()` from test/db package
func TangCoreDatabase(t *testing.T) *sqlx.DB {
	return tdb.TangCore(t)
}

// TangCoreDatabaseURL returns the database connection the url any test
// use when connecting to the tang-core database
//
// DEPRECATED:  use `TangCoreURL()` from test/db package
func TangCoreDatabaseURL() string {
	return tdb.TangCoreURL()
}
