...
func TestDefaultSubmitter(t *testing.T) {
	ctx := test.Context()
	// submits to the configured tang-core instance correctly
	...
	// Succeeds when tang-core gives the DUPLICATE response.
	server = test.NewStaticMockServer(`{
				"status": "DUPLICATE",
				"error": null
				}`)
	...
	// Errors when the tang-core url is empty

	s = NewDefaultSubmitter(http.DefaultClient, "")
	sr = s.Submit(ctx, "hello")
	assert.NotNil(t, sr.Err)

	//errors when the tang-core url is not parseable

	s = NewDefaultSubmitter(http.DefaultClient, "http://Not a url")
	sr = s.Submit(ctx, "hello")
	assert.NotNil(t, sr.Err)

	// errors when the tang-core url is not reachable
	s = NewDefaultSubmitter(http.DefaultClient, "http://127.0.0.1:65535")
	sr = s.Submit(ctx, "hello")
	assert.NotNil(t, sr.Err)

	// errors when the tang-core returns an unparseable response
	server = test.NewStaticMockServer(`{`)
	defer server.Close()
	...
	// errors when the tang-core returns an exception response
	...
	// errors when the tang-core returns an unrecognized status
	...
	// errors when the tang-core returns an error response
