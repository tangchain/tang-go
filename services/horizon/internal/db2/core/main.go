...
// reading rows from a Tang Core db
package core
...
// Offer is row of data from the `offers` table from tang-core
type Offer struct {
...
// Q is a helper struct on which to hang common queries against a tang
// core database.
type Q struct {
	*db.Session
}
...
// Signer is a row of data from the `signers` table from tang-core
type Signer struct {
	Accountid string
	Publickey string
	Weight    int32
...
// Transaction is row of data from the `txhistory` table from tang-core
...
// TransactionFee is row of data from the `txfeehistory` table from tang-core
...
// Trustline is a row of data from the `trustlines` table from tang-core
...
// tang-core database this ingestion system is communicating with.  Horizon,
