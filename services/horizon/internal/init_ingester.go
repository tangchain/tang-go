...
func initIngester(app *App) {
	...
	if app.networkPassphrase == "" {
		log.Fatal("Cannot start ingestion without network passphrase.  Please confirm connectivity with tang-core.")
	}

	app.ingester = ingest.New(
		app.networkPassphrase,
		app.config.TangCoreURL,
		app.CoreSession(nil),
...
func init() {
	appInit.Add("ingester", initIngester, "app-context", "log", "horizon-db", "core-db", "tangCoreInfo")
}
