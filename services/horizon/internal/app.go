...
import (
	...
	"github.com/tang/go/build"
	"github.com/tang/go/clients/tangcore"
	horizonContext "github.com/tang/go/services/horizon/internal/context"
	"github.com/tang/go/services/horizon/internal/db2/core"
	"github.com/tang/go/services/horizon/internal/db2/history"
	"github.com/tang/go/services/horizon/internal/ingest"
	"github.com/tang/go/services/horizon/internal/ledger"
	"github.com/tang/go/services/horizon/internal/operationfeestats"
	"github.com/tang/go/services/horizon/internal/paths"
	"github.com/tang/go/services/horizon/internal/reap"
...
// CoreSession returns a new session that loads data from the tang core
// database. The returned session is bound to `ctx`.
...
// UpdateTangCoreInfo updates the value of coreVersion and networkPassphrase
// from the Tang core API.
func (a *App) UpdateTangCoreInfo() {
	if a.config.TangCoreURL == "" {
		return
	}

	fail := func(err error) {
		log.Warnf("could not load tang-core info: %s", err)
	}

	core := &tangcore.Client{
		URL: a.config.TangCoreURL,
	}
...
func (a *App) Tick() {
	...
	// update ledger state, operation fee state, and tang-core info in parallel
	wg.Add(3)
	go func() { a.UpdateLedgerState(); wg.Done() }()
	go func() { a.UpdateOperationFeeStatsState(); wg.Done() }()
	go func() { a.UpdateTangCoreInfo(); wg.Done() }()
	wg.Wait()
