...
using JavaScript and `EventSource`.  We will eschew using [`js-tang-sdk`](https://github.com/tang/js-tang-sdk), the
...
```javascript
var Keypair = require("tang-base").Keypair;
...
The Tang test network provides the Friendbot, a tool that developers
can use to get testnet lumens for testing purposes. To fund your account, simply
execute the following curl command:

```bash
$ curl "https://friendbot.tang.org/?addr=GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
After a few seconds, the Tang network will perform consensus, close the
...
```bash
$ curl -H "Accept: text/event-stream" "https://horizon-testnet.tang.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments"
```
...
> If you need a reliable streaming connection please use our [SDK](https://github.com/tang/js-tang-sdk).
...
var es = new EventSource('https://horizon-testnet.tang.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments');
...
```bash
$ curl "https://horizon-testnet.tang.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
```js
var TangBase = require("tang-base");
TangBase.Network.useTestNetwork();

var keypair = TangBase.Keypair.fromSecret('SCU36VV2OYTUMDSSU4EIVX4UUHY3XC7N44VL4IJ26IOG6HVNC7DY5UJO');
var account = new TangBase.Account(keypair.publicKey(), "713226564141056");

var amount = "100";
var transaction = new TangBase.TransactionBuilder(account)
  .addOperation(TangBase.Operation.createAccount({
    destination: TangBase.Keypair.random().publicKey(),
...
After running this script you should see a signed transaction blob. To submit this transaction we send it to horizon or tang-core. But before we do, let's open a new console and start our previous script by `node stream_payments.js`.
