...
```sh
curl "https://horizon-testnet.tang.org/metrics"
```
...
Horizon maintains its own database (postgres), a verbose and user friendly account of activity on the Tang network.
...
#### Ingester
Ingester represents metrics specific to Horizon's [ingestion](https://github.com/tang/go/blob/master/services/horizon/internal/docs/reference/admin.md#ingesting-tang-core-data) process, or the process by which Horizon consumes transaction results from a connected Tang Core instance.
...
#### Tang Core
As noted above, Horizon relies on Tang Core to stay in sync with the Tang network. These metrics are specific to the underlying Tang Core instance.

|    Metric     |  Description                                                                                                                               |
| ---------------- |  ------------------------------------------------------------------------------------------------------------------------------ |
| latest_ledger    | The sequence number of the latest (most recent) ledger recorded in Tang Core's database.  |
| open_connections | The number of open connections to the Tang Core postgres database.  |

##### *Example Response:*
```shell
"tang_core.latest_ledger": {
  "value": 19203710
},
"tang_core.open_connections": {
  "value": 4
},
```
...
Horizon does not submit transactions directly to the Tang network. Instead, it sequences transactions and sends the base64 encoded, XDR serialized blob to its connected Tang Core instance. 

##### Horizon Transaction Sequencing and Submission

The following is a simplified version of the transaction submission process that glosses over the finer details. To dive deeper, check out the [source code](https://github.com/tang/go/tree/master/services/horizon/internal/txsub).

Horizon's sequencing mechanism consists of a [manager](https://github.com/tang/go/blob/master/services/horizon/internal/txsub/sequence/manager.go) that keeps track of [submission queues](https://github.com/tang/go/blob/master/services/horizon/internal/txsub/sequence/queue.go) for a set of addresses. A submission queue is a  priority queue, prioritized by minimum transaction sequence number, that holds a set of pending transactions for an account. A pending transaction is represented as an object with a sequence number and a channel. Periodically, this queue is updated, popping off finished transactions, sending down the transaction's channel a successful/failure response.
