...
import (
	...
	"github.com/tang/go/meta"
	"github.com/tang/go/services/horizon/internal/db2/history"
	"github.com/tang/go/services/horizon/internal/ingest/participants"
	"github.com/tang/go/support/errors"
	ilog "github.com/tang/go/support/log"
	sTime "github.com/tang/go/support/time"
...
func (is *Session) ingestEffects() {
	...
	case xdr.OperationTypeCreatePassiveOffer:
		...
		// KNOWN ISSUE:  tang-core creates results for CreatePassiveOffer operations
...
func (is *Session) ingestTrades() {
	...
	for i, trade := range trades {
		// tang-core will opportunisticly garbage collect invalid offers (in the
...
// reportCursorState makes an http request to the configured tang-core server
// to report that it has finished processing the data being ingested.  This
// allows tang-core to free that storage when next it runs its own
// maintenance.
func (is *Session) reportCursorState() error {
	...
	core := &tangcore.Client{URL: is.TangCoreURL}
