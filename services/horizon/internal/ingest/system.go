...
import (
	...
	herr "github.com/tang/go/services/horizon/internal/errors"
	"github.com/tang/go/services/horizon/internal/ledger"
	"github.com/tang/go/services/horizon/internal/toid"
	"github.com/tang/go/support/errors"
	ilog "github.com/tang/go/support/log"
)
...
// RebaseHistory re-establishes horizon's history database by clearing it, ingesting the latest ledger in tang-core then backfilling as many ledgers as possible
...
// run causes the importer to check tang-core to see if we can import new
// data.
func (i *System) runOnce() {
	...
	if coreLatest == 1 {
		log.Warn("ingest: waiting for tang-core sync")
		return
	}
...
// horizon but is no longer present in the tang-core database source.  The
// usual cause for this situation is a tang-core that uses the CATCHUP_RECENT
// mode.
