# bridge-server
This is a stand alone server written in go. It is designed to make connecting to the Tang network as easy as possible. 
...
- Creating Tang transactions.
- Monitoring a receiving Tang account.


## Downloading the server
[Prebuilt binaries](https://github.com/tang/bridge-server/releases) of the bridge-server server are available on the [releases page](https://github.com/tang/bridge-server/releases).

| Platform       | Binary file name                                                                         |
|----------------|------------------------------------------------------------------------------------------|
| Mac OSX 64 bit | [bridge-darwin-amd64](https://github.com/tang/bridge-server/releases)      |
| Linux 64 bit   | [bridge-linux-amd64](https://github.com/tang/bridge-server/releases)       |
| Windows 64 bit | [bridge-windows-amd64.exe](https://github.com/tang/bridge-server/releases) |

Alternatively, you can [build](#building) the binary yourself.

## Config

The `bridge.cfg` file must be present in a working directory (you can load another file by using `-c` parameter). Here is an [example configuration file](https://github.com/tang/bridge-server/blob/master/bridge_example.cfg). Config file should contain following values:
...
* `network_passphrase` - passphrase of the network that will be used with this bridge server:
   ...
   * public network: `Public Global Tang Network ; September 2015`
* `compliance` - URL to compliance server instance if you want to carry out the compliance protocol
* `horizon` - URL to [horizon](https://github.com/tang/horizon) server instance
...
* `mac_key` - a tang secret key used to add MAC headers to a payment notification.
...
* [`InternalServerError`](/src/github.com/tang/gateway/protocols/errors.go)

### POST /builder

Builds a transaction from a given request. `Content-Type` of this request should be `application/json`. Check [List of operations](https://www.tang.org/developers/learn/concepts/list-of-operations.html) doc to learn more about how each operation looks like.

**Note** This will not submit a transaction to the network. Please use [Horizon](https://www.tang.org/developers/horizon/reference/endpoints/transactions-create.html) to submit a transaction.
...
Assets are represented by a JSON object with two fields: `code` and `issuer`. Empty JSON object represents [native asset](https://www.tang.org/developers/learn/concepts/assets.html#lumens-xlm-).
...
* [`InternalServerError`](/src/github.com/tang/gateway/protocols/errors.go)
* [`InvalidParameterError`](/src/github.com/tang/gateway/protocols/errors.go)

### POST /payment

Builds and submits a transaction with a single [`payment`](https://www.tang.org/developers/learn/concepts/list-of-operations.html#payment), [`path_payment`](https://www.tang.org/developers/learn/concepts/list-of-operations.html#path-payment) or [`create_account`](https://www.tang.org/developers/learn/concepts/list-of-operations.html#create-account) (when sending native asset to account that does not exist) operation built from following parameters.

#### Safe transaction resubmittion

It’s possible that you will not receive a response from Bridge server due to a bug, network conditions, etc. In such situation it’s impossible to determine the status of your transaction and sending the same request to the Bridge server may result in "double-spend" of the funds. That’s why you should always send a request with `id` parameter set. Then when you resubmit a transaction with the same `id` the previously used [sequence number](https://www.tang.org/developers/guides/concepts/transactions.html) will be reused.
...
`sender` | optional | Payment address (ex. `bob*tang.org`) of payment sender account. Required for when sending using Compliance protocol.
`destination` | required | Account ID or payment address (ex. `bob*tang.org`) of payment destination account
...
`extra_memo` | optional | You can include any info here and it will be included in the pre-image of the transaction's memo hash. See the [Tang Memo Convention](https://github.com/tang/tang-protocol/issues/28). When set and compliance server is connected, `memo` and `memo_type` values will be ignored.
...
```
forward_destination[domain]=tang.org&forward_destination[fields][forward_type]=bank_account&forward_destination[fields][swift]=BOPBPHMM&forward_destination[fields][acct]=2382376
```
...
It will return [`SubmitTransactionResponse`](/src/github.com/tang/gateway/horizon/submit_transaction_response.go) if there were no errors or with one of the following errors:

* [`InternalServerError`](/src/github.com/tang/gateway/protocols/errors.go)
* [`InvalidParameterError`](/src/github.com/tang/gateway/protocols/errors.go)
* [`MissingParameterError`](/src/github.com/tang/gateway/protocols/errors.go)
* [`TransactionBadSequence`](/src/github.com/tang/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuth`](/src/github.com/tang/gateway/protocols/bridge/errors.go)
* [`TransactionInsufficientBalance`](/src/github.com/tang/gateway/protocols/bridge/errors.go)
* [`TransactionNoAccount`](/src/github.com/tang/gateway/protocols/bridge/errors.go)
* [`TransactionInsufficientFee`](/src/github.com/tang/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuthExtra`](/src/github.com/tang/gateway/protocols/bridge/errors.go)
* [`PaymentCannotResolveDestination`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentCannotUseMemo`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentSourceNotExist`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentAssetCodeNotAllowed`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentPending`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentDenied`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentMalformed`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentUnderfunded`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentSrcNoTrust`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentSrcNotAuthorized`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentNoDestination`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentNoTrust`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentNotAuthorized`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentLineFull`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentNoIssuer`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentTooFewOffers`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentOfferCrossSelf`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
* [`PaymentOverSendmax`](/src/github.com/tang/gateway/protocols/bridge/payment.go)
...
It will build and submits a transaction with a [`allow_trust`](https://www.tang.org/developers/learn/concepts/list-of-operations.html#allow-trust) operation. 
The source of this transaction will be the account specified by `accounts.authorizing_seed` config parameter. 
You should make sure that this account is a low weight signer on the issuing account. See [Multi-sig](https://www.tang.org/developers/learn/concepts/multi-sig.html) for more information. 
...
* [`TransactionInsufficientFee`](/src/github.com/tang/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuthExtra`](/src/github.com/tang/gateway/protocols/bridge/errors.go)
* [`AllowTrustMalformed`](/src/github.com/tang/gateway/protocols/bridge/authorize.go)
* [`AllowTrustNoTrustline`](/src/github.com/tang/gateway/protocols/bridge/authorize.go)
* [`AllowTrustTrustNotRequired`](/src/github.com/tang/gateway/protocols/bridge/authorize.go)
* [`AllowTrustCantRevoke`](/src/github.com/tang/gateway/protocols/bridge/authorize.go)
...
`data` | Value of the [AuthData](https://www.tang.org/developers/learn/integration-guides/compliance-protocol.html). This field will be empty when compliance server is not connected.
...
Then simply open:
```
http://localhost:6060/pkg/github.com/tang/gateway/
```
in a browser.
