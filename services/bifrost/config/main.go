...
type Config struct {
	...
	Tang struct {
		...
		IssuerPublicKey string `valid:"required,tang_accountid" toml:"issuer_public_key"`
		...
		DistributionPublicKey string `valid:"required,tang_accountid" toml:"distribution_public_key"`
		// SignerSecretKey is:
		...
		// https://www.tang.org/developers/guides/channels.html
		// Signer's sequence number will be consumed in transaction's sequence number.
		SignerSecretKey string `valid:"required,tang_seed" toml:"signer_secret_key"`
		...
		StartingBalance string `valid:"optional,tang_amount" toml:"starting_balance"`
...
func (c Config) SignerPublicKey() string {
	kp := keypair.MustParse(c.Tang.SignerSecretKey)
	return kp.Address()
}
