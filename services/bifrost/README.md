...
Bifrost is highly available and secure Bitcoin/Ethereum → Tang bridge. It allows users to move BTC/ETH to Tang network and then trade them to other tokens or participate in ICOs (initial coin offering).

It solves many problems connected to moving tokens to Tang network:

* Security:
  * Developers don’t have access to users’ Tang keys.
...
Download the binary from [the release page](https://github.com/tang/go/releases/tag/bifrost-v0.0.2) and use it with it's [Bifrost JS SDK](https://github.com/tang/bifrost-js-sdk).

## How it works

1. User opens the web app implemented using [Bifrost JS SDK](https://github.com/tang/bifrost-js-sdk).
1. User is presented with her public and private Tang keys where Bitcoin/Ethereum will be sent.
1. User selects what cryptocurrency she wants to move to Tang network.
1. A receiving Bitcoin/Ethereum address is generated.
1. User sends funds in Bitcoin/Ethereum network.
1. Bifrost listens to Bitcoin and Ethereum network events. When payment arrives it creates a Tang [account](https://www.tang.org/developers/guides/concepts/accounts.html) for the user who later adds a temporary signer on the account.
1. Using a temporary signer Bifrost creates necessary [trust lines](https://www.tang.org/developers/guides/concepts/assets.html) and sends corresponding amount of BTC/ETH to user's Tang account. It also exchanges BTC/ETH to the final token at a given rate.
...
1. Now open Bifrost demo: https://bifrost.tang.org/ It will display an Ethereum address for sending your test ETH.
...
* `bitcoin`
  ...
  * `token_price` - a price of one BTC in terms of final token (`tang.token_asset_code`)
* `ethereum`
  ...
  * `token_price` - a price of one ETH in terms of final token (`tang.token_asset_code`)
* `tang`
  ...
  * `signer_secret_key` - distribuions accounts's secret key if only one instance of Bifrost is deployed OR [channel](https://www.tang.org/developers/guides/channels.html)'s secret key if more than one instance of Bifrost is deployed, signer is also used as a temporary signer in new accounts (see "Account configuration" section)
  * `horizon` - URL to [horizon](https://github.com/tang/go/tree/master/services/horizon) server
  * `network_passphrase` - Tang network passphrase (`Public Global Tang Network ; September 2015` for production network, `Test SDF Network ; September 2015` for test network)
  * `starting_balance` - Tang XLM amount issued to created account (41 by default)
...
This is the most basic configuration but it is not recommended. In this configuration only one account is responsible for everything: issuing and distributing assets and creating new accounts. In this configuration `tang.issuer_public_key`, `tang.distribution_public_key` and `tang.signer_secret_key` represent a single account.

### Issuing and Distribution Accounts

Sometimes you want to have a limited supply of your token so you create a distribution account, send all the tokens to distribution (and other accounts, ex. vesting) and lock the issuing account so no new tokens can be created. In this configuration `tang.distribution_public_key` and `tang.signer_secret_key` represent a separate account used to distribute funds in a token sale. This account will also be used to create new accounts.

### Distribution Account with multiple signers

If you expect really high load you should deploy multiple Bifrost servers. To prevent Bifrost instances from consuming the same sequence numbers of the Distribution account you should use [channels](https://www.tang.org/developers/guides/channels.html). Each Bifrost server will contain a secret key of one of the signers of Distribution account and will use Signer account as a [transaction source](https://www.tang.org/developers/guides/concepts/transactions.html#source-account) when creating new accounts.

In this configuration all servers have the same `tang.issuer_public_key` and `tang.distribution_public_key` values but each server has a separate `tang.signer_secret_key` which is a signer of `tang.distribution_public_key` account (and needs to exist in Tang ledger).
...
* Make sure "Sell [your token] for BTC" and/or "Sell [your token] for ETH" exist in Tang production network. If not, you can create an offer by sending a transaction with `manage_offer` operation.
* Make sure you don't use account from `tang.issuer_secret_key` anywhere else than bifrost. Otherwise, sequence numbers will go out of sync and bifrost will stop working. It's good idea to create a new signer on issuing account.
...
* Deploy at least 2 bifrost, bitcoin-core, geth, tang-core and horizon servers. Use multi-AZ database.
...
* It's a good idea to set "Authorization required" and "Authorization revocable" [flags](https://www.tang.org/developers/guides/concepts/accounts.html#flags) on issuing account during ICO stage to remove trustlines to accounts with lost keys. You need to set `tang.needs_authorize` in config to `true` if you want to use this feature.
...
* `Bifrost.ExchangedTimelockedEvent` [event](https://github.com/tang/bifrost-js-sdk#oneventevent-data). You can display it to the user if you want your users to unlock accounts.
...
Check [Tang Stack Exchange](https://tang.stackexchange.com/questions/tagged/bifrost/) to find similar questions or add the new question with the `bifrost` tag.
