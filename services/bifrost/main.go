...
import (
	...
	"github.com/tang/go/services/bifrost/bitcoin"
	"github.com/tang/go/services/bifrost/config"
	"github.com/tang/go/services/bifrost/database"
	"github.com/tang/go/services/bifrost/ethereum"
	"github.com/tang/go/services/bifrost/server"
	"github.com/tang/go/services/bifrost/sse"
	"github.com/tang/go/services/bifrost/tang"
	"github.com/tang/go/services/bifrost/stress"
	supportConfig "github.com/tang/go/support/config"
...
var rootCmd = &cobra.Command{
	Use:   "bifrost",
	Short: "Bridge server to allow participating in Tang based ICOs using Bitcoin and Ethereum",
}
...
This command will create 3 server.Server's listening on ports 8000-8002.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		for i := 0; i < numServers; i++ {
			go func(i int) {
				cfg.Port = ports[i]
				cfg.Tang.SignerSecretKey = signers[i]
		...
		users := stress.Users{
			Horizon: &horizon.Client{
				URL: cfg.Tang.Horizon,
				HTTP: &http.Client{
					Timeout: 60 * time.Second,
				},
			},
			NetworkPassphrase: cfg.Tang.NetworkPassphrase,
			UsersPerSecond:    usersPerSecond,
			BifrostPorts:      ports,
			IssuerPublicKey:   cfg.Tang.IssuerPublicKey,
		}
...
func init() {
	// TODO I think these should be default in tang/go:
...
func createServer(cfg config.Config, stressTest bool) *server.Server {
	...
	tangAccountConfigurator := &tang.AccountConfigurator{
		NetworkPassphrase:     cfg.Tang.NetworkPassphrase,
		IssuerPublicKey:       cfg.Tang.IssuerPublicKey,
		DistributionPublicKey: cfg.Tang.DistributionPublicKey,
		SignerSecretKey:       cfg.Tang.SignerSecretKey,
		NeedsAuthorize:        cfg.Tang.NeedsAuthorize,
		TokenAssetCode:        cfg.Tang.TokenAssetCode,
		StartingBalance:       cfg.Tang.StartingBalance,
		LockUnixTimestamp:     cfg.Tang.LockUnixTimestamp,
	}

	if cfg.Tang.StartingBalance == "" {
		tangAccountConfigurator.StartingBalance = "2.1"
	}

	if cfg.Bitcoin != nil {
		tangAccountConfigurator.TokenPriceBTC = cfg.Bitcoin.TokenPrice
	}

	if cfg.Ethereum != nil {
		tangAccountConfigurator.TokenPriceETH = cfg.Ethereum.TokenPrice
	}
	...
	err = g.Provide(
		...
		&inject.Object{Value: tangAccountConfigurator},
	)
	if err != nil {
