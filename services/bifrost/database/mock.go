...
func (m *MockDatabase) CreateAddressAssociation(chain Chain, tangAddress, address string, addressIndex uint32) error {
	a := m.Called(chain, tangAddress, address, addressIndex)
	return a.Error(0)
}
...
func (m *MockDatabase) GetAssociationByTangPublicKey(tangPublicKey string) (*AddressAssociation, error) {
	a := m.Called(tangPublicKey)
	return a.Get(0).(*AddressAssociation), a.Error(1)
}
