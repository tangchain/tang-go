...
type Database interface {
	// CreateAddressAssociation creates Bitcoin/Ethereum-Tang association. `addressIndex`
	// is the chain (Bitcoin/Ethereum) address derivation index (BIP-32).
	CreateAddressAssociation(chain Chain, tangAddress, address string, addressIndex uint32) error
	// GetAssociationByChainAddress searches for previously saved Bitcoin/Ethereum-Tang association.
	// Should return nil if not found.
	GetAssociationByChainAddress(chain Chain, address string) (*AddressAssociation, error)
	// GetAssociationByTangPublicKey searches for previously saved Bitcoin/Ethereum-Tang association.
	// Should return nil if not found.
	GetAssociationByTangPublicKey(tangPublicKey string) (*AddressAssociation, error)
