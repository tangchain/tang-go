...
import (
	...
	"github.com/lib/pq"
	"github.com/tang/go/services/bifrost/queue"
...
type transactionsQueueRow struct {
	...
	TangPublicKey string          `db:"tang_public_key"`
}
...
func fromQueueTransaction(tx queue.Transaction) *transactionsQueueRow {
	return &transactionsQueueRow{
		...
		Amount:           tx.Amount,
		TangPublicKey: tx.TangPublicKey,
	}
}
...
func (d *PostgresDatabase) CreateAddressAssociation(chain Chain, tangAddress, address string, addressIndex uint32) error {
...
func (d *PostgresDatabase) GetAssociationByTangPublicKey(tangPublicKey string) (*AddressAssociation, error) {
	addressAssociationTable := d.getTable(addressAssociationTableName, nil)
	row := &AddressAssociation{}
	where := map[string]interface{}{"tang_public_key": tangPublicKey}
	err := addressAssociationTable.Get(row, where).Exec()
	if err != nil {
		switch errors.Cause(err) {
		case sql.ErrNoRows:
