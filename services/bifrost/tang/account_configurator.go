...
func (ac *AccountConfigurator) Start() error {
	ac.log = common.CreateLogger("TangAccountConfigurator")
	ac.log.Info("TangAccountConfigurator starting")
...
func (ac *AccountConfigurator) ConfigureAccount(destination, assetCode, amount string) {
	...
	localLog.Info("Configuring Tang account")
	...
	for {
		...
		localLog.WithField("destination", destination).Info("Creating Tang account")
		err = ac.createAccountTransaction(destination)
		if err != nil {
			localLog.WithField("err", err).Error("Error creating Tang account")
			time.Sleep(2 * time.Second)
			continue
		}
