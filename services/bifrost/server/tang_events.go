...
func (s *Server) onTangAccountCreated(destination string) {
	association, err := s.Database.GetAssociationByTangPublicKey(destination)
	if err != nil {
	...
	if association == nil {
		s.log.WithField("tangPublicKey", destination).Error("Association not found")
		return
	}
