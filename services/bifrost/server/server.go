...
import (
	...
	"github.com/tang/go/support/app"
...
func (s *Server) Start() error {
	...
	s.TangAccountConfigurator.OnAccountCreated = s.onTangAccountCreated
	s.TangAccountConfigurator.OnExchanged = s.onExchanged
	s.TangAccountConfigurator.OnExchangedTimelocked = s.OnExchangedTimelocked
	...
	err := s.TangAccountConfigurator.Start()
	if err != nil {
		return errors.Wrap(err, "Error starting TangAccountConfigurator")
	}
...
func (s *Server) handlerGenerateAddress(w stdhttp.ResponseWriter, r *stdhttp.Request, chain database.Chain) {
	...
	tangPublicKey := r.PostFormValue("tang_public_key")
	_, err := keypair.Parse(tangPublicKey)
	if err != nil || (err == nil && tangPublicKey[0] != 'G') {
		log.WithField("tangPublicKey", tangPublicKey).Warn("Invalid tangPublicKey")
		w.WriteHeader(stdhttp.StatusBadRequest)
		return
	}
	...
	err = s.Database.CreateAddressAssociation(chain, tangPublicKey, address, index)
	if err != nil {
		log.WithFields(log.F{
			"err":              err,
			"chain":            chain,
			"index":            index,
			"tangPublicKey": tangPublicKey,
			"address":          address,
