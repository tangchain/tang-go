...
import (
	...
	"github.com/tang/go/services/compliance/internal/handlers"
	supportConfig "github.com/tang/go/support/config"
	"github.com/tang/go/support/db/schema"
	"github.com/tang/go/support/errors"
	supportHttp "github.com/tang/go/support/http"
)
...
func init() {
	rootCmd = &cobra.Command{
		Use:   "compliance",
		Short: "tang compliance server",
		Long:  `tang compliance server`,
		Run:   run,
	}
...
func NewApp(config config.Config, migrateFlag bool, versionFlag bool, version string) (app *App, err error) {
	...
	err = g.Provide(
		...
		&inject.Object{Value: &tangtomlClient},
