...
* Fix for a bug in `/builder` endpoint: sequence number is now incremented when loaded from Horizon server (https://github.com/tang/bridge-server/issues/86).
...
* Added error messages to Compliance protocol ([SEP-0003](https://github.com/tang/tang-protocol/blob/master/ecosystem/sep-0003.md))

## 0.0.27

* Admin Panel (`/admin` endpoint in `bridge` server).
* `/tx_status` endpoint [More info](https://github.com/tang/tang-protocol/blob/master/ecosystem/sep-0001.md).
...
* [XLM (lumen)](https://www.tang.org/lumens/) payments can be now used in `PaymentListener`.
...
* Update `github.com/tang/go` dependency.
...
* Change tang.toml location to new standard.
...
* [Compliance protocol](https://www.tang.org/developers/learn/integration-guides/compliance-protocol.html) support.
...
* Sending to Tang address with memo in `/send`.
