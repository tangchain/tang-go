...
import (
	...
	"github.com/tang/go/protocols/federation"
...
func (rh *RequestHandler) HandlerSend(w http.ResponseWriter, r *http.Request) {
	...
	if authDataEntity != nil {
		tangToml, err := rh.TangTomlResolver.GetTangToml(authDataEntity.Domain)
		if err != nil {
		...
		if tangToml.AuthServer == "" {
			log.Print("No AUTH_SERVER in tang.toml")
			helpers.Write(w, callback.AuthServerNotDefined)
			return
		}

		rh.sendAuthData(w, tangToml.AuthServer, []byte(authDataEntity.AuthData))
		return
	}
	...
	tangToml, err := rh.TangTomlResolver.GetTangToml(domain)
	if err != nil {
		log.WithFields(log.Fields{
			"destination": request.Destination,
			"err":         err,
	...
	rh.sendAuthData(w, tangToml.AuthServer, data)
}
