...
import (
	...
	baseAmount "github.com/tang/go/amount"
	"github.com/tang/go/protocols/compliance"
	"github.com/tang/go/services/compliance/internal/db"
	shared "github.com/tang/go/services/internal/bridge-compliance-shared"
	httpHelpers "github.com/tang/go/services/internal/bridge-compliance-shared/http/helpers"
	callback "github.com/tang/go/services/internal/bridge-compliance-shared/protocols/compliance"
...
func (rh *RequestHandler) HandlerAuth(w http.ResponseWriter, r *http.Request) {
	...
	senderTangToml, err := rh.TangTomlResolver.GetTangTomlByAddress(authData.Sender)
	if err != nil {
		log.WithFields(log.Fields{"err": err, "sender": authData.Sender}).Warn("Cannot get tang.toml of sender")
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "Cannot get tang.toml of sender")
		httpHelpers.Write(w, errorResponse)
		return
	}

	if !shared.IsValidAccountID(senderTangToml.SigningKey) {
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "SIGNING_KEY in tang.toml of sender is invalid")
		// TODO
		// log.WithFields(errorResponse.LogData).Warn("SIGNING_KEY in tang.toml of sender is invalid")
		httpHelpers.Write(w, errorResponse)
		return
	}
	...
	err = rh.SignatureSignerVerifier.Verify(senderTangToml.SigningKey, []byte(authreq.DataJSON), signatureBytes)
	if err != nil {
		log.WithFields(log.Fields{
			"signing_key": senderTangToml.SigningKey,
			"data":        authreq.Data,
