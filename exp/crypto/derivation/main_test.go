...
func ExampleDeriveFromPath() {
	...
	key, err := DeriveForPath(TangPrimaryAccountPath, seed)
	if err != nil {
		panic(err)
	}
...
func ExampleDeriveMultipleKeys() {
	...
	for i := 0; i < 10; i++ {
		path := fmt.Sprintf(TangAccountPathFormat, i)
...
func ExampleDeriveMultipleKeysFaster() {
	seed, _ := hex.DecodeString("000102030405060708090a0b0c0d0e0f")
	mainKey, err := DeriveForPath(TangAccountPrefix, seed)
	if err != nil {
		panic(err)
	}
	...
	for i := uint32(0); i < 10; i++ {
		...
		fmt.Println(fmt.Sprintf(TangAccountPathFormat, i), kp.Seed(), kp.Address())
	}
...
func BenchmarkDerive(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := DeriveForPath(TangPrimaryAccountPath, seed)
		if err != nil {
			panic(err)
		}
	}
