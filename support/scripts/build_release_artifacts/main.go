...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/tang/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/tang/go/support/app.version=%s", version)
