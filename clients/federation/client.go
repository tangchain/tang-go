...
// LookupByAddress performs a federated lookup following to the tang
...
// "name" type is a legacy holdover from the legacy tang network's federation
...
// LookupByAccountID performs a federated lookup following to the tang
...
// ForwardRequest performs a federated lookup following to the tang
...
func (c *Client) getFederationServer(domain string) (string, error) {
	stoml, err := c.TangTOML.GetTangToml(domain)
	if err != nil {
		return "", errors.Wrap(err, "get tang.toml failed")
	}

	if stoml.FederationServer == "" {
		return "", errors.New("tang.toml is missing federation server info")
	}
