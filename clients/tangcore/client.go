...
// tang-core server using HTTP
type Client struct {
	// HTTP is the client to use when communicating with tang-core.  If nil,
	// http.DefaultClient will be used.
	HTTP HTTP

	// URL of Tang Core server to connect.
	URL string
}

// Info calls the `info` command on the connected tang core and returns the
// provided response
...
// SetCursor calls the `setcursor` command on the connected tang core
func (c *Client) SetCursor(ctx context.Context, id string, cursor int32) error {
	...
	if body != SetCursorDone {
		return errors.Errorf("failed to set cursor on tang-core: %s", body)
	}

	return nil
}

// SubmitTransaction calls the `tx` command on the connected tang core with the provided envelope
...
// WaitForNetworkSync continually polls the connected tang-core until it
...
// simpleGet returns a new GET request to the connected tang-core using the
